...
import { generateShortId } from 'utils/string'; // This function was defined here but moved to a different utils file.
    ...
    try {
        ...
        try {     
            ...
            if (nodes[stepIndex].data.type === 'trigger') {
                ...
                // TODO: Fix this sh1t, +1 is hard coded but should be getCronNodes => nodes.pop(0) so index 1 is now 0 but the rest of the indexes are present (delete index 0)
               ...
               const previousNodeId = nodes[stepIndex - 1].id;
               const previousNodeOutput = await getWorkflowNodeState(trigger_output);
